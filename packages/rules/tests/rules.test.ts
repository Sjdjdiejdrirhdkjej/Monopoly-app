...
import { BOARD, BOARD_SIZE, Phase, TileKind } from '@monopoly/shared'
...
const forcePosition = (s: any, pid: number, pos: number) => {
  s.players[pid].position = pos
}

const giveCash = (s: any, pid: number, amount: number) => {
  s.players[pid].cash = amount
}

const own = (s: any, pid: number, index: number) => {
...
describe('Rent calculations', () => {
  it('base and monopoly rents', () => {
    let s = start()
    const prop = BOARD.find(t => t.kind === TileKind.Property && t.color === 'Brown') as any
    ...
    const group = BOARD.filter(t => t.kind === TileKind.Property && (t as any).color === prop.color)
    for (const t of group) own(s, owner, t.index)
    const mono = selectors.rentFor(s, prop.index, s.lastRoll)
    expect(mono).toBe(base * 2)
  })
  it('houses and hotel rent', () => {
    let s = start()
    const prop = BOARD.find(t => t.kind === TileKind.Property && t.color === 'LightBlue') as any
    own(s, 0, prop.index)
