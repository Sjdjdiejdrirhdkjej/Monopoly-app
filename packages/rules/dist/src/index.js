...
import { BOARD_SIZE, BOARD, CardKind, Phase, RAILROADS, TileKind, UTILITIES, } from '@monopoly/shared';
import { createDecks } from '@monopoly/shared';
const VERSION = 1;
const defaultConfig = { startingCash: 1500, goAmount: 200, jailFine: 50 };
const clone = (x) => JSON.parse(JSON.stringify(x));
const createRNG = (seed, prngState, calls) => {
    ...
    return {
        seed,
        state: () => ({ seed, prngState: rng.state?.(), calls: c }),
...
const getOwnership = (s, i) => {
    if (!s.ownership[i])
        s.ownership[i] = { owner: null };
    return s.ownership[i];
};
const playerCount = (s) => s.players.filter(p => !p.bankrupt).length;
const nextPlayerIndex = (s) => {
...
const goTo = (s, pid, to, awardGoOnPass = true) => {
    const p = s.players[pid];
    const from = p.position;
    if (awardGoOnPass && to < from)
        awardGo(s, pid);
    p.position = to;
    addLog(s, { type: 'Moved', player: pid, from, to });
};
const sendToJail = (s, pid) => {
    const jailIndex = 10;
...
const ownsAllInGroup = (s, pid, color) => {
...
const railroadsOwned = (s, pid) => RAILROADS.filter(i => getOwnership(s, i).owner === pid).length;
const utilitiesOwned = (s, pid) => UTILITIES.filter(i => getOwnership(s, i).owner === pid).length;
const rentFor = (s, i, dice, cardUtilityMultiplier) => {
...
const pay = (s, from, to, amount, reason) => {
    const p = s.players[from];
    if (p.cash >= amount) {
        p.cash -= amount;
        if (to !== 'Bank')
            s.players[to].cash += amount;
...
const resolveLanding = (s, rng, pid) => {
...
const applyCard = (s, rng, pid, card) => {
    ...
    if (card.kind === CardKind.PayEachPlayer) {
        for (const other of s.players)
            if (other.id !== pid && !other.bankrupt)
                pay(s, pid, other.id, card.arg, 'CardPayEach');
        s.phase = Phase.EndTurn;
        return;
    }
...
export const generateLegalActions = (s) => {
    if (s.phase === Phase.Setup)
        return [{ type: 'StartGame', seed: 'seed', players: [] }];
    ...
    if (s.trade?.active && s.trade.offer)
        return [{ type: 'AcceptTrade' }, { type: 'RejectTrade' }];
    if (s.phase === Phase.PreRoll) {
        if (p.inJail) {
            const acts = [{ type: 'TryDoubles' }];
            if (p.cash >= s.config.jailFine)
                acts.push({ type: 'PayJail' });
    ...
    if (s.phase === Phase.BuyDecision)
        return [{ type: 'Buy' }, { type: 'DeclineBuy' }];
    if (s.phase === Phase.EndTurn)
        return [{ type: 'EndTurn' }];
    return [];
};
export const apply = (state, action) => {
    ...
    if (action.type === 'StartGame') {
        ...
        for (const t of s.board)
            if (t.kind === TileKind.Property || t.kind === TileKind.Railroad || t.kind === TileKind.Utility)
                ownership[t.index] = { owner: null, houses: 0, hotel: false, mortgaged: false };
        const decks = createDecks(seed);
        const nextState = {
            ...s,
            version: VERSION,
            rng: rng2.state(),
    ...
    if (s.trade?.active && s.trade.offer) {
        ...
        if (action.type === 'AcceptTrade') {
            ...
            if (from.cash >= offer.cashFrom && to.cash >= offer.cashTo) {
                ...
                for (const t of offer.tilesFrom)
                    transferTile(s, t, offer.to);
                for (const t of offer.tilesTo)
                    transferTile(s, t, offer.from);
    ...
    if (action.type === 'Build') {
        ...
        if (tile.kind !== TileKind.Property)
            throw new Error('Can only build on properties');
        const groupTiles = BOARD.filter(t => t.kind === TileKind.Property && t.color === tile.color).map(t => t.index);
        if (!groupTiles.every(i => getOwnership(s, i).owner === pid))
            throw new Error('Need monopoly');
        const houses = own.houses ?? 0;
        if (own.hotel)
            throw new Error('Already hotel');
        const minH = Math.min(...groupTiles.map(i => getOwnership(s, i).houses ?? 0));
        if (houses > minH)
            throw new Error('Even-building rule');
    ...
    if (action.type === 'Sell') {
        ...
        const tile = s.board[action.tile];
        if (tile.kind !== TileKind.Property)
            throw new Error('Can only sell on properties');
        const groupTiles = BOARD.filter(t => t.kind === TileKind.Property && t.color === tile.color).map(t => t.index);
